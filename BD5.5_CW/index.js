const express = require('express')
const app = express();
const PORT = 3000;
const { sequelize } = require('./lib/index.js');
const { Op } = require('@sequelize/core');
const { user } = require('./models/user.model.js');
const { track } = require('./models/track.model.js');
const { like } = require('./models/like.model.js');


app.use(express.json())
//data
const users = [
  {
    username: 'testuser',
    email: 'testuser@gmail.com',
    password: 'testuser',
  }
]

const tracks = [
    {
      name: 'Raabta',
      genre: 'Romantic',
      release_year: 2012,
      artist: 'Arijit Singh',
      album: 'Agent Vinod',
      duration: 4,
    },
    {
      name: 'Naina Da Kya Kasoor',
      genre: 'Pop',
      release_year: 2018,
      artist: 'Amit Trivedi',
      album: 'Andhadhun',
      duration: 3,
    },
    {
      name: 'Ghoomar',
      genre: 'Traditional',
      release_year: 2018,
      artist: 'Shreya Ghoshal',
      album: 'Padmaavat',
      duration: 3,
    },
    {
      name: 'Bekhayali',
      genre: 'Rock',
      release_year: 2019,
      artist: 'Sachet Tandon',
      album: 'Kabir Singh',
      duration: 6,
    },
    {
      name: 'Hawa Banke',
      genre: 'Romantic',
      release_year: 2019,
      artist: 'Darshan Raval',
      album: 'Hawa Banke (Single)',
      duration: 3,
    },
    {
      name: 'Ghungroo',
      genre: 'Dance',
      release_year: 2019,
      artist: 'Arijit Singh',
      album: 'War',
      duration: 5,
    },
    {
      name: 'Makhna',
      genre: 'Hip-Hop',
      release_year: 2019,
      artist: 'Tanishk Bagchi',
      album: 'Drive',
      duration: 3,
    },
    {
      name: 'Tera Ban Jaunga',
      genre: 'Romantic',
      release_year: 2019,
      artist: 'Tulsi Kumar',
      album: 'Kabir Singh',
      duration: 3,
    },
    {
      name: 'First Class',
      genre: 'Dance',
      release_year: 2019,
      artist: 'Arijit Singh',
      album: 'Kalank',
      duration: 4,
    },
    {
      name: 'Kalank Title Track',
      genre: 'Romantic',
      release_year: 2019,
      artist: 'Arijit Singh',
      album: 'Kalank',
      duration: 5,
    },
  ]

//functions
async function likeTrack(data){
  
  let newLike = await like.create(
    {
      userId: data.userId, 
      trackId: data.trackId
    });
  return {message : "Track Liked", newLike};
}

async function dislikeTrack(data){
  let like = await like.destroy({
    where : {
      userId : data.userId,
      trackId : data.trackId
    }
  });
  return {message : "Track Disliked", like};
}

async function getAllLikedTracks(userId) {
  let getTrackId = await like.findAll({
    where: {
      userId: userId
    },
    attributes: ['trackId']
  });
  let likedTracks = [];
  for (let i = 0; i < getTrackId.length; i++){
    likedTracks.push(getTrackId[i].trackId)
  }
  let trackRecords = await track.findAll({
    where : { id : { [Op.in] : likedTracks } }
  });
  return { trackRecords };
}

async function getAllLikedTracksByArtists(data){
  let getTrackId = await like.findAll({
    where: {
      userId: data.userId
    },
    attributes: ['trackId']
  });
  let likedTracks = [];
  for (let i = 0; i < getTrackId.length; i++){
    likedTracks.push(getTrackId[i].trackId)
  }
  let trackRecords = await track.findAll({
    where : { id : { [Op.in] : likedTracks } , artist : data.artist }
  });
  return { trackRecords };
}

//endpoints
//seed data
app.get("/seed_data", async (req, res) =>{
  try{
    await sequelize.sync( { force: true } );
    await user.bulkCreate(users);
    await track.bulkCreate(tracks);
    res.json({message: 'Data seeding successfull'});
  }catch(error){
    res.status(500).json({ message: error.message })
  }
})

app.get("/", (req, res) => {
  res.status(200).json({message : "Welcome to the BD5.5_CW"})
})

//users/:id/like
app.get("/users/:id/like", async (req, res) =>{
  let userId = req.params.id;
  let trackId = req.query.trackId;
  try{
    let result = await likeTrack({ userId, trackId });
    res.status(200).json(result)
  }catch(err){
    res.status(500).json({message : err.message})
  }
})

//users/:id/dislike
app.get("/users/:id/dislike", async (req, res) =>{
  let userId = req.params.id;
  let trackId = req.query.trackId;
  try{
    let result = await dislikeTrack({ userId, trackId });
    res.status(200).json(result)
  }catch(err){
    res.status(500).json({message : err.message})
  }
})

//users/:id/liked
app.get("/users/:id/liked", async (req, res) =>{
  let userId = req.params.id;
  try{
    let result = await getAllLikedTracks(userId);
    if(result.trackRecords.length === 0){
      res.status(404).json({message : "No liked tracks found"})
    }
    res.status(200).json(result)
  }catch(err){
    res.status(500).json({message : err.message})
  }
})

//users/:id/liked-artist
app.get("/users/:id/liked-artist", async (req, res) =>{
  let userId = req.params.id;
  let artist = req.query.artist;
  try{
    let response = await getAllLikedTracksByArtists({userId, artist});
    if(response.trackRecords.length === 0){
      res.status(404).json({message : "No liked tracks found"})
    }
    res.status(200).json(response)
  }catch(err){
    res.status(500).json({message : err.message})
  }
})


app.listen(PORT, () => {
  console.log("Server is running on port 3000");
})